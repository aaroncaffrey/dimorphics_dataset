...
namespace dimorphics_dataset
{
    ...
    {
        public static List<subsequence_classification_data> find_coils(string dimer_type, string pdb_id, int coils_class_id = -1, int min_coil_length_required = 2, bool use_dssp3 = true)
        {
            var pdb_atoms = atom.load_atoms_pdb(pdb_id, true, true, false, true, true, true).Where(a => a.pdb_model_index == 0).SelectMany(a => a.pdb_model_chain_atoms).ToList();
        ...
        public static List<subsequence_classification_data> run_coil_dataset_maker(int coils_class_id = -1, List<(string dimer_type, string pdb_id)> pdb_id_list = null, int min_coil_length_required = 3, bool use_dssp3 = true)//, bool limit_for_testing = false)
        {
            var tasks = new List<Task<List<subsequence_classification_data>>>();
            ...
            foreach (var loop_pdb_id in pdb_id_list)
            {
                ...
                var task = Task.Run(() =>
                {
                    var pdb_coils = find_coils(pdb_id.dimer_type, pdb_id.pdb_id);
