...
namespace dimorphics_dataset
{
    internal class protein_subsequence_info
    {
        ...
        {
            var text = psi_list.Select(a => /*program.string_debug*/($@"{a.class_id},{a.class_name},{a.dimer_type},{a.parallelism},{a.symmetry_mode},{a.pdb_id},{a.chain_id},{string.Join(/*program.string_debug*/($@";"), a.res_ids.Select(b => /*program.string_debug*/($@"{b.amino_acid}{b.i_code}{b.res_id}")).ToList())},{a.aa_subsequence}")).ToList();
            text.Insert(0, /*program.string_debug*/($@"{nameof(class_id)},{nameof(class_name)},{nameof(dimer_type)},{nameof(parallelism)},{nameof(symmetry_mode)},{nameof(pdb_id)},{nameof(chain_id)},{nameof(res_ids)},{nameof(aa_subsequence)}"));
